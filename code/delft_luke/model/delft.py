    ...
    def propagate(self, g):
        ...
        if self.is_input_layer:
            ...
            def msg_func(edges):
                ...
                return {'evi_sent_rep': evidence_sent_rep, 'score': evi_sent_score} #dictionary, score of evidence
            ...
            def msg_func(edges):
                ...
                evi_sent_score = self.evidence_score(evidence_sent_rep, edges.src['hq']) #bilinear, 1 output
                #embedding and score
        ...
        def reduce_func(nodes):
            #mailbox? dgl
            #visit all the information , possible functions like sum, min, max
            sc = nodes.mailbox['sc']
            v = nodes.mailbox['msg']
            c = nodes.mailbox['c']
...
#Encode question sentence rnn, gated recurrent neural network
class Encoder_rnn(nn.Module):
    ...
    def forward(self, alias, alias_len):  
        ...
        unpacked, unpacked_len = torch.nn.utils.rnn.pad_packed_sequence(output) #B*T**
...
class LinearAttn(nn.Module):
    ...
    def forward(self, x, x_mask):
        ...
        #x = x.transpose(0, 1)
        output_avg = alpha.unsqueeze(1).bmm(x).squeeze(1) #matrix multiplication 3 dimension (b,h,w)/(b,w,h)
...
class BilinearSeqAttn(nn.Module):  # for the evidence edge
...
#Final DELFT Model
'''https://docs.dgl.ai/tutorials/models/1_gnn/4_rgcn.html?highlight=pop'''
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.final_layer = torch.nn.Sequential(
            ...
            nn.Dropout(0.5),
            nn.ReLU(),
            nn.Linear(self.args.hidden_size, 1)    #hidden size to one output 1, logits
        )
    ...
    def forward(self, batch):
        ...
        g.ndata['question'] = g.ndata['question'].to(self.device) # returns a dict[str, Tensor]
